    ...
    >
      ...
      <div class="p-5 space-y-4">
        <div class="text-xs text-zinc-300 leading-relaxed">
          FlowGit đã phát hiện <strong class="text-white">{mergedBranches.length}</strong> nhánh local đã được gộp hoàn toàn vào nhánh chính (<code class="text-cyan-300 font-mono">main</code>/<code class="text-cyan-300 font-mono">master</code>).
        </div>

        {#if mergedBranches.length === 0}
          <div class="p-6 rounded-xl bg-zinc-950 border border-zinc-800 text-center space-y-2">
            <GitBranch class="w-6 h-6 text-emerald-400 mx-auto" />
            <div class="text-xs font-bold text-emerald-300">Không có nhánh rác!</div>
        ...
        {:else}
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between text-xs text-zinc-400 px-1">
              <button
                type="button"
                onclick={toggleAll}
                class="hover:text-white font-medium cursor-pointer"
              >
            ...
            <div class="max-h-52 overflow-y-auto space-y-1 p-2 bg-zinc-950 border border-zinc-800 rounded-xl">
              {#each mergedBranches as branch}
                <button
                  type="button"
                  onclick={() => toggleBranch(branch)}
                  class="w-full flex items-center justify-between p-2 rounded-lg text-xs cursor-pointer transition-colors {selectedBranches.includes(branch) ? 'bg-rose-950/30 border border-rose-800/40 text-rose-200' : 'bg-zinc-900/40 border border-zinc-800/40 text-zinc-400 hover:bg-zinc-800/40'}"
                >
                  <div class="flex items-center gap-2 font-mono truncate">
                    <GitBranch class="w-3.5 h-3.5 text-zinc-500" />
                    <span>{branch}</span>
                  </div>
                  <div class="w-4 h-4 rounded border flex items-center justify-center shrink-0 {selectedBranches.includes(branch) ? 'bg-rose-500 border-rose-500 text-white' : 'border-zinc-700'}">
          ...
          <!-- Safe Note -->
          <div class="p-3 rounded-xl bg-zinc-950/80 border border-zinc-800/80 flex items-start gap-2.5 text-[11px] text-zinc-400">
