...
{#if show && branch}
  <div class="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
    <div
      class="bg-zinc-900 border border-zinc-700/80 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-150"
    >
      <!-- Header -->
      <div class="flex items-center justify-between px-5 py-4 border-b border-zinc-800 bg-zinc-950/40">
        <div class="flex items-center gap-2.5">
          {#if isProtected}
            ...
            <h2 class="text-sm font-semibold text-zinc-100">Nhánh được bảo vệ (Protected Branch)</h2>
          {:else}
        ...
        <button
          onclick={onClose}
          disabled={isLoading}
          class="text-zinc-400 hover:text-white p-1 rounded-lg hover:bg-zinc-800 transition-colors cursor-pointer disabled:opacity-50"
        >
          <X class="w-4 h-4" />
        </button>
      </div>
      ...
      <div class="p-5 space-y-4 text-xs">
        <!-- Branch Details Card -->
        <div class="p-3.5 rounded-lg bg-zinc-950/60 border border-zinc-800/80 space-y-2">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2 font-mono font-medium text-zinc-200">
              {#if branch.is_remote}
                <Globe class="w-3.5 h-3.5 text-purple-400 shrink-0" />
              {:else}
                <GitBranch class="w-3.5 h-3.5 text-cyan-400 shrink-0" />
              {/if}
              <span class="truncate text-sm">{branch.shorthand}</span>
            </div>
            <span
          ...
          <div class="text-[11px] text-zinc-500 font-mono truncate">
            Target Commit: <span class="text-zinc-400">{branch.target_commit_id?.slice(0, 8)}</span>
          </div>
        </div>

        {#if isProtected}
          <!-- Protected Branch Notice -->
          <div class="p-3.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-200 space-y-2">
            <div class="flex items-start gap-2.5">
              <ShieldAlert class="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
              <div class="space-y-1">
                <p class="font-medium text-amber-300">Không thể xóa nhánh này</p>
                <p class="text-[11px] text-amber-200/80 leading-relaxed">
                  Nhánh <code class="font-mono font-semibold bg-amber-950/60 px-1 py-0.5 rounded text-amber-300">{branch.shorthand}</code> 
        ...
        {:else}
          ...
          {#if !branch.is_remote}
            ...
            {#if branch.ahead_count > 0}
              <div class="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-300 flex items-start gap-2.5">
            ...
            {:else}
              <p class="text-zinc-400 leading-relaxed">
                Bạn có chắc chắn muốn xóa nhánh cục bộ <span class="font-mono text-zinc-200 font-semibold">{branch.shorthand}</span>? Thao tác này sẽ gỡ bỏ nhánh khỏi danh sách Local Branches.
              </p>
            {/if}
          {:else}
            ...
            <div class="space-y-3">
              <p class="text-zinc-400 leading-relaxed">
                Bạn đang thực hiện xóa tham chiếu nhánh Remote <span class="font-mono text-zinc-200 font-semibold">{branch.shorthand}</span>.
              </p>
              ...
              <div class="p-3 rounded-lg bg-zinc-950 border border-zinc-800 space-y-2">
                <label class="flex items-start gap-2.5 cursor-pointer select-none">
                  <input
                    ...
                    class="mt-0.5 rounded border-zinc-700 bg-zinc-900 text-rose-500 focus:ring-rose-500/20 focus:ring-offset-0 cursor-pointer"
                  />
                  <div class="space-y-0.5">
                    <span class="text-xs font-semibold text-rose-300">Đồng thời xóa trên máy chủ Remote (GitHub/GitLab)</span>
      ...
      <!-- Actions -->
      <div class="flex items-center justify-end gap-2 px-5 py-3.5 border-t border-zinc-800/80 bg-zinc-950/40">
        <button
          onclick={onClose}
          disabled={isLoading}
          class="px-3.5 py-1.5 rounded-lg text-xs font-medium text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition-colors cursor-pointer disabled:opacity-50"
        >
