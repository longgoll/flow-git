    ...
    >
      ...
      <div class="px-6 py-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/60">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-xl bg-teal-500/10 border border-teal-500/30 flex items-center justify-center text-teal-400 shadow-inner">
      ...
      <div class="px-6 py-3.5 bg-zinc-950/40 border-b border-zinc-800 flex items-center justify-between">
        <div>
          ...
          <div class="flex items-center gap-2 mt-0.5">
            ...
            {#if currentRepoIdentity?.is_local}
              <span class="px-1.5 py-0.5 rounded text-[10px] bg-teal-950/60 text-teal-300 border border-teal-800/60 font-mono">
                Local .git/config
              </span>
            {:else}
              <span class="px-1.5 py-0.5 rounded text-[10px] bg-zinc-800 text-zinc-400 font-mono">
                Kế thừa Global
              </span>
            {/if}
          </div>
        ...
        <button
          ...
          class="px-2.5 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-xs text-zinc-200 font-medium flex items-center gap-1.5 transition-colors cursor-pointer"
        >
          <Plus class="w-3.5 h-3.5 text-teal-400" />
          <span>Tạo hồ sơ</span>
        </button>
      </div>
      ...
      {#if showAddForm}
        <div class="px-6 py-4 bg-zinc-950/90 border-b border-zinc-800 space-y-3 animate-in slide-in-from-top duration-150">
          <h3 class="text-xs font-bold text-zinc-300">Thêm hồ sơ mới</h3>
          <div class="grid grid-cols-3 gap-2">
            <input
              type="text"
              bind:value={newLabel}
              placeholder="Tên nhãn (vd: Open Source)"
              class="px-3 py-1.5 bg-zinc-900 border border-zinc-700 rounded-lg text-xs text-zinc-100 placeholder:text-zinc-600 outline-hidden"
            />
            <input
              type="text"
              bind:value={newName}
          ...
          <div class="flex items-center justify-end gap-2">
            <button
              ...
              class="px-3 py-1 rounded-lg text-zinc-400 hover:text-zinc-200 text-xs cursor-pointer"
            >
              Hủy
            </button>
            <button
              onclick={handleSaveNewProfile}
              class="px-3 py-1 rounded-lg bg-teal-600 hover:bg-teal-500 text-white text-xs font-semibold cursor-pointer transition-colors"
            >
              Lưu hồ sơ
            </button>
          </div>
      ...
      <div class="p-6 max-h-[420px] overflow-y-auto space-y-2.5">
        ...
        {:else}
          {#each profiles as profile (profile.id)}
            ...
            <div
              class="p-3.5 rounded-xl border transition-all flex items-center justify-between gap-3 {isCurrent ? 'bg-teal-950/20 border-teal-700/60 shadow-xs' : 'bg-zinc-950/70 border-zinc-800 hover:border-zinc-700'}"
            >
              ...
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="truncate">
                  <div class="flex items-center gap-2">
                    ...
                    {#if isCurrent}
                      <span class="px-2 py-0.5 rounded-full text-[10px] bg-teal-500/20 text-teal-300 font-bold flex items-center gap-1 border border-teal-500/30">
                  ...
                  <div class="flex items-center gap-2 text-xs text-zinc-400 mt-0.5">
                    ...
                    <span class="font-mono text-zinc-300 text-[11px]">{profile.email}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-2 shrink-0">
                {#if !isCurrent}
                  <button
                    ...
                    class="px-3 py-1.5 rounded-lg bg-teal-600/20 hover:bg-teal-600/30 text-teal-300 text-xs font-semibold border border-teal-500/40 transition-colors cursor-pointer"
                  >
                    Áp dụng
                  </button>
                {/if}
      ...
      <div class="px-6 py-3.5 border-t border-zinc-800 bg-zinc-950/60 flex items-center justify-between">
        <label class="flex items-center gap-2 cursor-pointer text-xs text-zinc-400 select-none">
          <input
            ...
            class="rounded border-zinc-700 bg-zinc-800 text-teal-600 focus:ring-0 cursor-pointer"
          />
