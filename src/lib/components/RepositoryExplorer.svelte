...
<div class="h-full w-full flex bg-zinc-950 text-zinc-200 overflow-hidden select-none">
  <!-- Left Panel: File Tree Explorer (300px) -->
  <div class="w-80 h-full border-r border-zinc-800/80 bg-zinc-950/80 flex flex-col shrink-0">
    <!-- Explorer Header & Commit Context Switcher -->
    <div class="p-3 border-b border-zinc-800/80 bg-zinc-900/40 flex flex-col gap-2 shrink-0">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <FolderGit2 class="w-4 h-4 text-cyan-400" />
          <span class="font-bold text-xs text-zinc-100">Files Explorer</span>
        </div>
        <button
          onclick={refreshTree}
        ...
        >
          <RefreshCw class="w-3.5 h-3.5 {isTreeLoading ? 'animate-spin text-cyan-400' : ''}" />
        </button>
      </div>
      ...
      <div class="flex items-center gap-1.5 text-[11px] font-mono">
        ...
        <select
          ...
          class="flex-1 bg-zinc-900 border border-zinc-800 rounded px-2 py-1 text-zinc-300 text-xs focus:outline-none focus:border-cyan-500 cursor-pointer truncate"
        >
      ...
      <div class="relative w-full">
        <Search class="w-3 h-3 text-zinc-500 absolute left-2.5 top-2 pointer-events-none" />
        <input
          type="text"
          bind:value={treeSearchQuery}
          placeholder="Filter files..."
          class="w-full bg-zinc-900/80 border border-zinc-800 rounded pl-7 pr-2 py-1 text-xs text-zinc-300 placeholder-zinc-500 focus:outline-none focus:border-cyan-500 font-mono"
        />
      </div>
    </div>
          ...
          {#each filtered as item (item.path)}
            {#if item.is_dir}
              ...
              <div>
                <button
                  ...
                  class="w-full flex items-center gap-1.5 py-1 px-1.5 rounded hover:bg-zinc-900 text-zinc-300 hover:text-white transition-colors cursor-pointer text-left"
                  style="padding-left: {depth * 14 + 6}px"
                >
                  {#if isExpanded}
                    <ChevronDown class="w-3 h-3 text-zinc-500 shrink-0" />
                    <FolderOpen class="w-3.5 h-3.5 text-amber-400 shrink-0" />
                  {:else}
                    <ChevronRight class="w-3 h-3 text-zinc-500 shrink-0" />
                ...
                {#if isExpanded}
                  ...
                  {:else}
                    <div class="py-1 text-[10px] text-zinc-600 italic" style="padding-left: {(depth + 1) * 14 + 6}px">
                      Loading...
                    </div>
                  {/if}
                {/if}
            ...
            {:else}
              ...
              <button
                ...
                class="w-full flex items-center justify-between py-1 px-1.5 rounded text-left transition-colors cursor-pointer {isSelected ? 'bg-cyan-950/60 text-cyan-300 font-semibold border border-cyan-800/50' : 'hover:bg-zinc-900 text-zinc-400 hover:text-zinc-200'}"
                style="padding-left: {depth * 14 + 20}px"
              >
                <div class="flex items-center gap-1.5 truncate min-w-0">
                  <IconComponent class="w-3.5 h-3.5 {isSelected ? 'text-cyan-400' : 'text-zinc-500'} shrink-0" />
                  <span class="truncate text-[11px]">{item.name}</span>
                </div>
                {#if item.size !== undefined && item.size !== null}
                  <span class="text-[9px] text-zinc-600 shrink-0 pl-1">{formatBytes(item.size)}</span>
                {/if}
              </button>
            {/if}
          {/each}
  ...
  <div class="flex-1 h-full flex flex-col overflow-hidden bg-zinc-950">
    {#if selectedFilePath}
      <!-- File Code Header -->
      <div class="h-10 px-4 border-b border-zinc-800/80 bg-zinc-900/60 flex items-center justify-between shrink-0 select-none">
        <!-- Path & File Details -->
        <div class="flex items-center gap-2 truncate">
          <FileCode class="w-4 h-4 text-cyan-400 shrink-0" />
          <span class="font-bold text-xs text-zinc-100 truncate font-mono">{selectedFilePath}</span>

          <div class="flex items-center gap-1.5 ml-2 font-mono text-[11px]">
            {#if fileSize > 0}
              <span class="px-1.5 py-0.2 rounded bg-zinc-800 text-zinc-400 border border-zinc-700/60 text-[10px]">
                {formatBytes(fileSize)}
              </span>
            {/if}
            {#if selectedCommitOid}
              <span class="px-1.5 py-0.2 rounded bg-indigo-950/60 text-indigo-300 border border-indigo-800/50 text-[10px] flex items-center gap-1">
                <GitCommit class="w-2.5 h-2.5" />
                <span>{selectedCommitOid.slice(0, 7)}</span>
              </span>
            {:else}
              <span class="px-1.5 py-0.2 rounded bg-emerald-950/60 text-emerald-300 border border-emerald-800/50 text-[10px] flex items-center gap-1">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="px-2 py-1 rounded flex items-center gap-1 text-[11px] border border-zinc-800 hover:bg-zinc-800 transition-colors cursor-pointer {wordWrap === 'on' ? 'text-cyan-400 border-cyan-800/60' : 'text-zinc-400'}"
          ...
          <button
            ...
            class="px-2 py-1 rounded flex items-center gap-1 text-[11px] border border-zinc-800 hover:bg-zinc-800 transition-colors cursor-pointer {minimap ? 'text-cyan-400 border-cyan-800/60' : 'text-zinc-400'}"
          ...
          {#if !isBinary}
            <button
              onclick={toggleBlame}
              class="px-2 py-1 rounded flex items-center gap-1 text-[11px] border border-zinc-800 hover:bg-zinc-800 transition-colors cursor-pointer {showBlame ? 'text-purple-400 border-purple-800/60 bg-purple-950/40 font-medium' : 'text-zinc-400'}"
            ...
            <button
              ...
              class="px-2 py-1 rounded flex items-center gap-1 text-[11px] border border-zinc-800 hover:bg-zinc-800 transition-colors cursor-pointer text-zinc-400 hover:text-zinc-200"
          ...
          <button
            onclick={copyPath}
            class="px-2 py-1 rounded bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-300 hover:text-white text-[11px] flex items-center gap-1 cursor-pointer transition-colors"
          ...
          <button
            onclick={copyContent}
            class="px-2.5 py-1 rounded bg-cyan-950/60 hover:bg-cyan-900/80 border border-cyan-800/60 text-cyan-300 text-[11px] flex items-center gap-1 font-medium cursor-pointer transition-colors"
          ...
          {#if onNukeFile && selectedFilePath}
            <button
              ...
              class="px-2 py-1 rounded bg-rose-950/40 hover:bg-rose-900/60 border border-rose-800/40 text-rose-300 hover:text-white text-[11px] flex items-center gap-1 cursor-pointer transition-colors"
      ...
      <div class="flex-1 relative overflow-hidden">
        ...
        {:else if isBinary}
          <div class="h-full flex flex-col items-center justify-center text-zinc-500 gap-3 select-none">
            <Binary class="w-12 h-12 text-cyan-400 opacity-60" />
            <div class="text-center">
              <p class="text-sm font-semibold text-zinc-300">Binary File Detected</p>
              <p class="text-xs text-zinc-600 mt-1">This file cannot be displayed as plain text in the code viewer.</p>
            </div>
          </div>
        {:else}
          <div class="w-full h-full flex overflow-hidden">
            {#if showBlame}
              ...
              <div class="w-80 bg-zinc-950 border-r border-zinc-800/80 shrink-0 flex flex-col font-mono text-xs select-none">
                <div class="px-3 py-1.5 bg-zinc-900/90 border-b border-zinc-800 text-[11px] text-zinc-400 font-semibold flex items-center justify-between">
                  <div class="flex items-center gap-1.5 text-purple-300">
                    <UserCheck class="w-3.5 h-3.5" />
                    <span>Git Blame</span>
                  </div>
                  <span class="text-[10px] text-zinc-500">{blameHunks.length} hunks</span>
                </div>
                <div class="flex-1 overflow-y-auto divide-y divide-zinc-800/50">
                  ...
                  {:else if blameHunks.length === 0}
                    <div class="p-4 text-center text-zinc-500 text-[11px]">
                  ...
                  {:else}
                    {#each blameHunks as hunk}
                      ...
                      <div
                        class="px-2.5 py-1.5 hover:bg-zinc-900/80 transition-colors cursor-pointer group"
                      ...
                      >
                        <div class="flex items-center justify-between text-[11px]">
                          <span class="text-purple-400 font-bold group-hover:underline">
                            {hunk.final_short_id}
                          </span>
                        ...
                        <div class="flex items-center justify-between text-[10px] text-zinc-400 mt-0.5">
                          <span class="truncate max-w-[140px] text-zinc-300 font-medium">
                            {hunk.author_name}
                          </span>
                        ...
                        <p class="text-[10px] text-zinc-500 truncate mt-0.5 group-hover:text-zinc-300">
    ...
    {:else}
      ...
      <div class="h-full flex flex-col items-center justify-center text-zinc-500 gap-3 select-none">
        <FolderGit2 class="w-10 h-10 text-zinc-700" />
        <div class="text-center space-y-1">
          <p class="text-xs text-zinc-400 font-medium">Select a file from the repository tree</p>
          <p class="text-[11px] text-zinc-600">Full source code preview with VS Code Monaco Engine</p>
        </div>
      </div>
    {/if}
  </div>
...
{#if contextMenu}
  ...
  ></div>
  <div
    class="fixed z-50 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl p-1 w-56 text-xs text-zinc-200 divide-y divide-zinc-800 font-sans"
  ...
  >
    <div class="py-1">
      <button
        ...
        class="w-full text-left px-2.5 py-1.5 rounded-lg hover:bg-zinc-800 flex items-center gap-2 cursor-pointer transition-colors"
      >
    ...
    {#if onNukeFile}
      <div class="py-1">
        <button
          ...
          class="w-full text-left px-2.5 py-1.5 rounded-lg hover:bg-rose-950/60 text-rose-400 hover:text-rose-200 flex items-center gap-2 cursor-pointer transition-colors"
        >
