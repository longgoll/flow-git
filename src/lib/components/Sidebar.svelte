...
<aside class="w-60 h-full bg-zinc-950/80 border-r border-zinc-800/60 flex flex-col select-none overflow-hidden shrink-0 transition-all duration-200">
  <!-- Repository Overview Card -->
  <div class="p-3 border-b border-zinc-800/60 bg-zinc-900/30">
    <div class="flex items-center justify-between text-zinc-300 font-medium text-xs">
      <div class="flex items-center gap-1.5 truncate">
        <FolderGit2 class="w-3.5 h-3.5 text-cyan-400 shrink-0" />
        <span class="truncate font-semibold">{repoSummary?.name || 'No Repository'}</span>
      </div>
      {#if onCloseSidebar}
        <button
          onclick={onCloseSidebar}
          class="p-1 rounded hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer"
    ...
    {#if repoSummary}
      ...
      <div class="mt-2 flex items-center gap-2">
        <div class="flex items-center gap-1 text-[10px] text-zinc-400 font-mono">
          <span class="w-1.5 h-1.5 rounded-full {repoSummary.dirty_files_count > 0 ? 'bg-amber-400' : 'bg-emerald-400'}"></span>
          <span>{repoSummary.dirty_files_count} unstaged</span>
        </div>
        {#if repoSummary.staged_files_count > 0}
          <span class="text-zinc-600">•</span>
          <div class="text-[10px] text-emerald-400 font-medium font-mono">
  ...
  <div class="flex-1 overflow-y-auto p-2 space-y-3 font-sans">
    ...
    {#if worktrees.length > 0}
      <div>
        <button
          ...
          class="w-full flex items-center justify-between px-2 py-1 text-[11px] font-bold uppercase tracking-wider text-zinc-400 hover:text-zinc-200 transition-colors cursor-pointer"
        >
          <div class="flex items-center gap-1.5">
            <FolderGit2 class="w-3.5 h-3.5 text-purple-400" />
            <span>Worktrees</span>
            <span class="text-[10px] text-zinc-500 font-mono">({worktrees.length})</span>
          </div>
          {#if showWorktrees}
          ...
          <div class="mt-1 space-y-0.5 pl-1">
            {#each worktrees as wt}
              <button
                ...
                class="w-full flex items-center justify-between px-2 py-1.5 rounded-md text-xs transition-colors cursor-pointer text-zinc-300 hover:bg-zinc-900/80 hover:text-white group"
              >
                <div class="flex items-center gap-2 truncate pr-1">
                  <span class="w-1.5 h-1.5 rounded-full {wt.is_main ? 'bg-cyan-400' : 'bg-purple-400'} shrink-0"></span>
                  <span class="truncate font-mono text-[11px]">{wt.name}</span>
                </div>
                {#if wt.branch_name}
                  <span class="text-[10px] font-mono text-zinc-500 truncate max-w-20">{wt.branch_name}</span>
                {/if}
              </button>
            {/each}
          </div>
    ...
    <div>
      <div
        class="w-full flex items-center justify-between px-2 py-1 text-[11px] font-bold uppercase tracking-wider text-zinc-400 hover:text-zinc-200 transition-colors"
      >
        <button
          onclick={() => (showLocalBranches = !showLocalBranches)}
          class="flex items-center gap-1.5 cursor-pointer flex-1 text-left"
        >
          <GitBranch class="w-3.5 h-3.5 text-cyan-400" />
          <span>Local Branches</span>
          <span class="text-[10px] text-zinc-500 font-mono">({localBranches.length})</span>
        </button>
        <div class="flex items-center gap-1">
          {#if onCreateBranch}
            <button
              onclick={(e) => { e.stopPropagation(); onCreateBranch(); }}
              class="p-0.5 rounded hover:bg-zinc-800 text-zinc-500 hover:text-cyan-300 transition-colors cursor-pointer"
          ...
          {#if onCleanMergedBranches}
            <button
              ...
              class="p-0.5 rounded hover:bg-zinc-800 text-zinc-500 hover:text-rose-400 transition-colors cursor-pointer"
        ...
        <div class="mt-1 space-y-0.5 pl-1">
          {#each localBranches as branch (branch.name)}
            ...
            <div
              ...
              class="w-full flex items-center justify-between px-2 py-1.5 rounded-md text-xs transition-colors group {branch.is_head ? 'bg-cyan-950/40 text-cyan-200 font-semibold border border-cyan-800/40' : 'text-zinc-300 hover:bg-zinc-900/80 hover:text-white'}"
            >
              {#if editingBranchName === branch.shorthand}
                ...
                <div class="flex items-center gap-1.5 flex-1 pr-1" onclick={(e) => e.stopPropagation()}>
                  ...
                  <input
                    ...
                    class="w-full px-1.5 py-0.5 text-[11px] font-mono bg-zinc-900 text-cyan-200 border border-cyan-500 rounded outline-hidden focus:ring-1 focus:ring-cyan-400"
                    placeholder="New branch name..."
                  />
                  <button
                    onclick={() => handleConfirmRename(branch)}
                    class="p-0.5 rounded bg-emerald-950/80 text-emerald-400 hover:bg-emerald-900 border border-emerald-600/50 cursor-pointer"
                  ...
                  <button
                    ...
                    class="p-0.5 rounded bg-zinc-800 text-zinc-400 hover:bg-zinc-700 cursor-pointer"
                    title="Hủy (Esc)"
                  >
              ...
              {:else}
                ...
                >
                  ...
                  {:else}
                    <span class="w-1.5 h-1.5 rounded-full bg-zinc-600 group-hover:bg-zinc-400 shrink-0"></span>
                  {/if}
              ...
              <div class="flex items-center gap-1 shrink-0">
                ...
                {#if !branch.upstream_name}
                  <button
                    ...
                    disabled={isPushing}
                    class="flex items-center gap-1 px-1.5 py-0.5 rounded bg-cyan-950/80 hover:bg-cyan-900 border border-cyan-700/60 text-cyan-300 hover:text-white text-[10px] font-medium transition-all cursor-pointer shadow-xs disabled:opacity-50"
                    title={`Publish nhánh "${branch.shorthand}" lên remote origin (git push -u origin ${branch.shorthand})`}
                  >
                    <CloudUpload class="w-3 h-3 text-cyan-400 {isPushing ? 'animate-bounce' : ''}" />
                    <span class="text-[9px] font-mono font-semibold">Publish</span>
                  </button>
                {:else}
                  <!-- Ahead / Behind Badge -->
                  {#if branch.ahead_count > 0 || branch.behind_count > 0}
                    <div class="flex items-center gap-1 text-[10px] font-mono px-1.5 py-0.5 rounded bg-zinc-800/80 text-zinc-400 shrink-0">
                  ...
                  {#if onPushBranch}
                    <button
                      ...
                      class="{branch.ahead_count > 0
                        ? 'flex p-1 rounded bg-emerald-950/70 hover:bg-emerald-900/90 border border-emerald-700/50 text-emerald-400 hover:text-emerald-200 cursor-pointer shadow-xs'
                        : 'opacity-0 group-hover:opacity-25 hidden group-hover:flex p-1 rounded text-zinc-500 cursor-not-allowed'} transition-all"
                  ...
                  {#if onFetchBranch}
                    <button
                      ...
                      class="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-cyan-950/60 text-zinc-500 hover:text-cyan-400 transition-all cursor-pointer"
                ...
                <button
                  ...
                  class="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300 transition-all cursor-pointer"
                ...
                {#if isProtectedBranch(branch)}
                  {#if !branch.is_head}
                    <div
                      class="opacity-0 group-hover:opacity-100 p-1 text-amber-400/70"
                ...
                {:else if onDeleteBranch}
                  <button
                    ...
                    class="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-rose-950/60 text-zinc-500 hover:text-rose-400 transition-all cursor-pointer"
          ...
          {#if localBranches.length === 0}
            <div class="px-2 py-1 text-[11px] text-zinc-600 italic">No local branches</div>
          {/if}
        </div>
      {/if}
    </div>
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <span class="text-[10px] text-zinc-500 font-mono">({remoteBranches.length})</span>
        </button>
        <div class="flex items-center gap-1">
          {#if onFetchPrune}
            <button
              onclick={(e) => { e.stopPropagation(); onFetchPrune(); }}
              class="p-0.5 rounded hover:bg-zinc-800 text-zinc-500 hover:text-purple-300 transition-colors cursor-pointer"
      ...
      {#if showRemoteBranches}
        <div class="mt-1 space-y-0.5 pl-1">
          {#each remoteBranches as branch (branch.name)}
            <div
              class="w-full flex items-center justify-between px-2 py-1.5 rounded-md text-xs text-zinc-400 hover:bg-zinc-900/60 hover:text-zinc-200 transition-colors group"
            >
              ...
              {#if isProtectedBranch(branch)}
                <div
                  class="opacity-0 group-hover:opacity-100 p-1 text-purple-400/60"
              ...
              {:else if onDeleteBranch}
                <button
                  onclick={(e) => { e.stopPropagation(); onDeleteBranch(branch); }}
                  class="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-rose-950/60 text-zinc-500 hover:text-rose-400 transition-all cursor-pointer shrink-0"
          ...
          {#if remoteBranches.length === 0}
            <div class="px-2 py-1 text-[11px] text-zinc-600 italic">No remote branches</div>
          {/if}
        </div>
      {/if}
    </div>
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <span>Remotes</span>
          <span class="text-[10px] text-zinc-500 font-mono">({remotes.length})</span>
        </button>
        <div class="flex items-center gap-1">
      ...
      {#if showRemotes}
        <div class="mt-1 space-y-0.5 pl-1">
          {#each remotes as remote (remote.name)}
            <div class="flex items-center justify-between px-2 py-1 rounded-md text-xs text-zinc-400 hover:bg-zinc-900/60 hover:text-zinc-200 font-mono text-[11px] group">
              <div class="flex items-center gap-2 truncate" title={remote.fetch_url}>
                ...
                <span class="truncate font-semibold text-zinc-300">{remote.name}</span>
              </div>
              <div class="flex items-center gap-1 shrink-0">
                {#if onFetchRemote}
                  <button
                    onclick={() => onFetchRemote(remote.name)}
                    class="opacity-0 group-hover:opacity-100 p-0.5 rounded text-zinc-500 hover:text-cyan-400 hover:bg-zinc-800 transition-all cursor-pointer"
                ...
                {#if onOpenRemoteManager}
                  <button
                    ...
                    class="opacity-0 group-hover:opacity-100 p-0.5 rounded text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 transition-all cursor-pointer"
            ...
            <div class="px-2 py-2 flex flex-col gap-2">
              ...
              {#if onPublishRepo}
                <button
                  ...
                  class="w-full py-1.5 px-2.5 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 border border-indigo-500/30 hover:border-indigo-500/50 rounded-lg text-[11px] font-medium flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs"
                  title="Xuất bản dự án này lên GitHub (chọn Công khai hoặc Riêng tư)"
                >
                  <CloudUpload class="w-3.5 h-3.5 text-indigo-400" />
    ...
    <div>
      ...
      >
        <div class="flex items-center gap-1.5">
          ...
          <span>Tags</span>
          <span class="text-[10px] text-zinc-500 font-mono">({tags.length})</span>
        </div>
        {#if showTags}
          <ChevronDown class="w-3.5 h-3.5 text-zinc-500" />
        {:else}
      ...
      {#if showTags}
        <div class="mt-1 space-y-0.5 pl-1">
            ...
            <div class="flex items-center justify-between px-2 py-1 rounded-md text-xs text-zinc-400 hover:bg-zinc-900/60 hover:text-zinc-200 font-mono text-[11px] group">
              ...
              {#if onDeleteTag}
                <button
                  ...
                  class="opacity-0 group-hover:opacity-100 p-0.5 rounded text-zinc-500 hover:text-rose-400 hover:bg-zinc-800 transition-all cursor-pointer"
          ...
          {/each}
          {#if tags.length === 0}
            <div class="px-2 py-1 text-[11px] text-zinc-600 italic">No tags</div>
          {/if}
        </div>
      {/if}
    </div>
    ...
    <div>
      ...
      >
        <div class="flex items-center gap-1.5">
          <Archive class="w-3.5 h-3.5 text-rose-400" />
          <span>Stashes</span>
          <span class="text-[10px] text-zinc-500 font-mono">({stashes.length})</span>
        </div>
        {#if showStashes}
          <ChevronDown class="w-3.5 h-3.5 text-zinc-500" />
        {:else}
      ...
      {#if showStashes}
        <div class="mt-1 space-y-0.5 pl-1">
          {#each stashes as stash (stash.index)}
            <div class="flex items-center gap-2 px-2 py-1.5 rounded-md text-xs text-zinc-400 hover:bg-zinc-900/60 hover:text-zinc-200 font-mono text-[11px]">
              <span class="text-rose-400 font-semibold">stash@{`{${stash.index}}`}</span>
              <span class="truncate text-zinc-500">{stash.message}</span>
            </div>
          {/each}
          {#if stashes.length === 0}
            <div class="px-2 py-1 text-[11px] text-zinc-600 italic">No stashes</div>
          {/if}
        </div>
      {/if}
    </div>
...
{#if activeBranchMenu}
  ...
  >
    <div
      class="fixed w-56 bg-zinc-900/95 border border-zinc-800 rounded-xl shadow-2xl backdrop-blur-xl p-1 z-50 text-xs font-sans animate-in fade-in zoom-in-95 duration-100"
      style="left: {activeBranchMenu.x}px; top: {activeBranchMenu.y}px;"
      onclick={(e) => e.stopPropagation()}
    >
      <div class="px-2.5 py-1.5 border-b border-zinc-800/80 mb-1">
        <div class="font-mono text-[11px] font-bold text-zinc-200 truncate flex items-center gap-1.5">
          <GitBranch class="w-3.5 h-3.5 text-cyan-400 shrink-0" />
      ...
      {#if !activeBranchMenu.branch.is_head}
        {#if onSelectBranch}
          <button
            ...
            class="w-full flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-zinc-800 text-zinc-300 hover:text-white transition-colors cursor-pointer text-left"
          >
        ...
        {#if onRebaseBranch}
          <button
            ...
            class="w-full flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-amber-950/60 text-amber-300 hover:text-amber-100 transition-colors cursor-pointer text-left font-medium"
      ...
      {#if !activeBranchMenu.branch.upstream_name}
        {#if onPublishBranch}
          <button
            ...
            class="w-full flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-cyan-950/60 text-cyan-300 hover:text-cyan-100 transition-colors cursor-pointer text-left font-medium"
          >
            <CloudUpload class="w-3.5 h-3.5 text-cyan-400" />
      ...
      {:else}
        {#if onPushBranch}
          <button
            ...
            class="w-full flex items-center justify-between px-2 py-1.5 rounded-md transition-colors text-left {activeBranchMenu.branch.ahead_count > 0 ? 'hover:bg-zinc-800 text-zinc-300 hover:text-white cursor-pointer' : 'text-zinc-600 cursor-not-allowed opacity-50'}"
          ...
          >
            <div class="flex items-center gap-2">
              <Upload class="w-3.5 h-3.5 {activeBranchMenu.branch.ahead_count > 0 ? 'text-emerald-400' : 'text-zinc-600'}" />
              <span>Push lên {activeBranchMenu.branch.upstream_name}</span>
            </div>
            {#if activeBranchMenu.branch.ahead_count > 0}
              <span class="text-[10px] font-mono text-emerald-400 font-bold">+{activeBranchMenu.branch.ahead_count}</span>
            {:else}
              <span class="text-[9px] font-mono text-zinc-600">Up to date</span>
            {/if}
          </button>
          <button
            onclick={() => {
            ...
            class="w-full flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-amber-950/50 text-amber-300 hover:text-amber-200 transition-colors cursor-pointer text-left"
          >
      ...
      {#if !isProtectedBranch(activeBranchMenu.branch) && onDeleteBranch}
        ...
        <button
          ...
          class="w-full flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-rose-950/60 text-rose-400 hover:text-rose-200 transition-colors cursor-pointer text-left"
        >
