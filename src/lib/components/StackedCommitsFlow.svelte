...
<div class="flex-1 flex flex-col w-full h-full min-h-0 bg-zinc-950 overflow-hidden font-sans select-none">
  <!-- Top Banner -->
  <div class="px-5 py-3 border-b border-indigo-900/40 bg-gradient-to-r from-indigo-950/40 via-zinc-950 to-zinc-950 flex items-center justify-between gap-4 shrink-0 shadow-sm">
    <div class="flex items-center gap-3">
      <div class="w-8 h-8 rounded-lg bg-indigo-500/20 border border-indigo-500/40 flex items-center justify-center text-indigo-400 shrink-0">
        <Box class="w-4 h-4" />
      </div>

      <div>
        <div class="flex items-center gap-2">
          <span class="text-sm font-bold text-zinc-100">Stacked Commits Flow</span>
          <span class="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 font-mono">
    ...
    <!-- Actions & Safe Badge -->
    <div class="flex items-center gap-3">
      <div class="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded bg-emerald-950/40 border border-emerald-800/40 text-emerald-400 text-xs font-mono">
      ...
      <button
        onclick={onClose}
        class="p-1.5 rounded-md hover:bg-zinc-800 text-zinc-400 hover:text-white transition-colors cursor-pointer"
  ...
  {#if statusMessage}
    <div class="px-5 py-2 bg-indigo-950/30 border-b border-indigo-900/40 text-xs font-mono text-indigo-200 flex items-center justify-between">
      <span>{statusMessage}</span>
      <button onclick={() => (statusMessage = '')} class="text-zinc-500 hover:text-zinc-300 text-[10px]">Dismiss</button>
    </div>
  {/if}
  ...
  <div class="flex-1 overflow-y-auto p-6 max-w-2xl mx-auto w-full">
    ...
    {:else if commits.length > 0}
      <div class="space-y-3">
        <div class="text-[11px] font-mono text-zinc-500 uppercase tracking-wider flex items-center justify-between pb-1 border-b border-zinc-800/80">
        ...
        {#each commits as commit, idx}
          ...
          <div
            ...
            class="relative group rounded-lg border bg-zinc-900/60 p-3.5 transition-all flex items-start gap-3 select-none {isDragging ? 'opacity-40 scale-95 border-indigo-500' : isTarget ? 'border-indigo-400 bg-indigo-950/30 shadow-lg' : 'border-zinc-800/80 hover:border-zinc-700/80 hover:bg-zinc-900'}"
          >
            <!-- Drag Handle -->
            <div class="pt-1 text-zinc-600 group-hover:text-zinc-400 cursor-grab active:cursor-grabbing shrink-0" title="Kéo để đổi thứ tự commit">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-2 mb-1">
                <div class="flex items-center gap-2">
                  <span class="px-2 py-0.5 rounded bg-zinc-950 border border-zinc-800 font-mono text-cyan-400 text-[11px] font-bold">
                    {commit.short_id}
                  </span>
                  {#if idx === 0}
                    <span class="px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-[10px] font-mono font-bold">
                      HEAD
                    </span>
                  {/if}
                </div>
                ...
                <div class="flex items-center gap-3 text-zinc-500 text-[11px] font-mono">
                  <div class="flex items-center gap-1">
                    <User class="w-3 h-3 text-zinc-600" />
              ...
              <p class="text-xs text-zinc-200 font-medium font-sans leading-snug">
                {commit.summary}
              </p>
            </div>
            ...
            <div class="flex flex-col gap-1 shrink-0 pt-0.5">
              <button
                ...
                disabled={idx === 0}
                class="p-1 rounded hover:bg-zinc-800 text-zinc-500 hover:text-zinc-200 disabled:opacity-20 disabled:pointer-events-none transition-colors cursor-pointer"
    ...
    {:else}
      <div class="h-64 flex flex-col items-center justify-center text-center p-8 text-zinc-500 text-xs gap-3">
        <Box class="w-10 h-10 text-zinc-600" />
        <span class="text-zinc-300 font-semibold text-sm">All commits are pushed</span>
