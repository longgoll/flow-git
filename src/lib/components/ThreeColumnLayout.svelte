...
<div class="flex-1 flex w-full h-full min-h-0 bg-zinc-950 overflow-hidden divide-x divide-zinc-800/80">
  <!-- Column 1: Commits List & Graph (35%) -->
  <div class="w-[36%] min-w-[320px] max-w-[500px] h-full flex flex-col min-h-0 overflow-hidden bg-zinc-950/60">
    <div class="h-8 px-3 border-b border-zinc-800/80 flex items-center justify-between bg-zinc-900/60 shrink-0 select-none">
      <span class="text-[11px] font-semibold tracking-wide text-zinc-400 uppercase">Commits & Graph</span>
      <span class="text-[10px] font-mono text-cyan-400 bg-cyan-950/60 border border-cyan-800/60 px-1.5 py-0.5 rounded">
  ...
  <div class="w-[24%] min-w-[240px] max-w-[380px] h-full flex flex-col min-h-0 overflow-hidden bg-zinc-950/90 border-r border-zinc-800/80">
    {#if isDetailLoading}
    ...
    {:else if commitDetail}
      ...
      <div class="p-3 border-b border-zinc-800/80 bg-zinc-900/40 space-y-2 shrink-0">
        <div class="flex items-center justify-between gap-2">
          <!-- Commit Hash -->
          <button
            onclick={copyHash}
            class="flex items-center gap-1.5 px-2 py-0.5 rounded bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white font-mono text-[11px] transition-colors cursor-pointer"
          ...
          >
            ...
            {:else}
              <Copy class="w-2.5 h-2.5 text-zinc-500" />
            {/if}
          </button>
        ...
        <div class="flex items-center gap-1.5 text-xs text-zinc-300">
          <div class="w-4 h-4 rounded-full bg-cyan-600/30 text-cyan-300 flex items-center justify-center text-[10px] font-bold shrink-0">
        ...
        <!-- Summary / Message -->
        <div class="max-h-24 overflow-y-auto bg-zinc-950/80 p-2 rounded border border-zinc-800/60">
          <p class="text-xs text-zinc-200 font-sans leading-relaxed whitespace-pre-wrap select-text">
      ...
      <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
        <div class="p-2 border-b border-zinc-800/80 bg-zinc-900/30 flex items-center justify-between gap-2 shrink-0">
          <div class="flex items-center gap-1.5">
            <span class="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Changed Files</span>
            <span class="text-[10px] font-mono bg-zinc-800 text-zinc-300 px-1.5 py-0.2 rounded-full">
          ...
          {#if commitDetail.files_changed.length > 5}
            <div class="relative w-32">
              <Search class="w-3 h-3 text-zinc-500 absolute left-1.5 top-2" />
              <input
                type="text"
                bind:value={fileSearchQuery}
                placeholder="Filter files..."
                class="w-full bg-zinc-900 border border-zinc-800 rounded pl-6 pr-2 py-0.5 text-[11px] text-zinc-200 focus:outline-none focus:border-cyan-500/50"
              />
            </div>
          {/if}
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-1.5 space-y-0.5">
          {#each renderedFiles as file}
            ...
            <button
              ...
              class="w-full text-left px-2 py-1.5 rounded flex items-center justify-between text-xs font-mono transition-all cursor-pointer group {isSelected ? 'bg-cyan-600/20 border border-cyan-500/40 text-cyan-200' : 'hover:bg-zinc-900/80 text-zinc-300 border border-transparent'}"
            >
              ...
              <div class="flex items-center gap-1.5 shrink-0">
                <span class="text-[9px] uppercase px-1 py-0.2 rounded border {file.status === 'added' ? 'border-emerald-800/80 text-emerald-400 bg-emerald-950/40' : file.status === 'deleted' ? 'border-rose-800/80 text-rose-400 bg-rose-950/40' : 'border-zinc-800 text-zinc-400 bg-zinc-900'}">
                  {file.status.slice(0, 3)}
                </span>
                <ChevronRight class="w-3 h-3 opacity-0 group-hover:opacity-100 {isSelected ? 'opacity-100 text-cyan-400' : 'text-zinc-500'}" />
              </div>
            </button>
          {/each}
          ...
          {#if visibleFiles.length > fileDisplayLimit}
            <button
              ...
              class="w-full py-2 mt-1 rounded bg-zinc-900/80 hover:bg-zinc-800 text-[11px] font-mono text-cyan-400 text-center transition-colors cursor-pointer border border-zinc-800"
            >
          ...
          {#if visibleFiles.length === 0}
            <div class="p-4 text-center text-xs text-zinc-600 italic">
    ...
    {:else}
      <div class="h-full flex flex-col items-center justify-center text-center p-6 text-zinc-600 text-xs">
        <GitCommit class="w-8 h-8 mb-2 opacity-40 text-zinc-500" />
  ...
  <div class="flex-1 h-full min-w-0 overflow-hidden flex flex-col bg-zinc-950">
    <DiffViewer
