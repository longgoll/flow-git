  ...
  <div class="flex items-center gap-1.5 sm:gap-2 min-w-0 shrink">
    ...
    {#if onToggleSidebar}
      <button
        ...
        class="p-1.5 rounded-md hover:bg-zinc-800/80 text-zinc-400 hover:text-zinc-100 transition-colors cursor-pointer shrink-0"
    ...
    <!-- Logo / App Name -->
    <div class="flex items-center gap-1.5 sm:gap-2 font-bold tracking-tight text-white select-none shrink-0">
      <div class="w-6 h-6 rounded-lg bg-gradient-to-tr from-cyan-600 via-teal-500 to-emerald-400 flex items-center justify-center shadow-md shadow-cyan-950/50">
        <FolderGit2 class="w-3.5 h-3.5 text-zinc-950 stroke-[2.5]" />
      </div>
      <span class="text-sm bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-teal-200 hidden 2xl:inline">FlowGit</span>
    </div>
    ...
    <div class="flex items-center bg-zinc-900/90 border border-zinc-800/80 rounded-lg p-0.5 shrink-0 shadow-inner">
      ...
      <button
        ...
        class="px-1.5 sm:px-2 py-1 rounded-md flex items-center gap-1.5 text-xs font-medium transition-all cursor-pointer {viewMode === 'focus' ? 'bg-amber-600 text-white shadow-sm font-semibold' : 'text-zinc-400 hover:text-zinc-200'}"
      ...
      <button
        ...
        class="px-1.5 sm:px-2 py-1 rounded-md flex items-center gap-1.5 text-xs font-medium transition-all cursor-pointer {viewMode === 'stacked' ? 'bg-indigo-600 text-white shadow-sm font-semibold' : 'text-zinc-400 hover:text-zinc-200'}"
      ...
      <button
        ...
        class="px-1.5 sm:px-2 py-1 rounded-md flex items-center gap-1.5 text-xs font-medium transition-all cursor-pointer {viewMode === 'dag' ? 'bg-teal-600 text-white shadow-sm font-semibold' : 'text-zinc-400 hover:text-zinc-200'}"
  ...
  <div class="flex items-center gap-1 sm:gap-1.5 shrink-0 min-w-0 justify-end">
    {#if viewMode === 'graph'}
      ...
      {#if onChangeLayoutMode}
        <div class="hidden 2xl:flex items-center bg-zinc-900/80 border border-zinc-800/80 rounded-md p-0.5 shrink-0" title="Chuyển đổi bố cục Graph">
          <button
            onclick={() => onChangeLayoutMode?.('horizontal')}
            class="p-1 rounded text-xs transition-all cursor-pointer {layoutMode === 'horizontal' ? 'bg-zinc-800 text-cyan-400 shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}"
          ...
          <button
            ...
            class="p-1 rounded text-xs transition-all cursor-pointer {layoutMode === 'three-column' ? 'bg-zinc-800 text-cyan-400 shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}"
    ...
    {#if onOpenPalette}
      ...
      >
        ...
        <kbd class="text-[9px] font-mono text-zinc-500 bg-zinc-800 px-1 py-0.2 rounded border border-zinc-700/60">⌘K</kbd>
      </button>
    {/if}
    ...
    <button
      onclick={onRefresh}
      disabled={isLoading}
      class="p-1.5 rounded-md hover:bg-zinc-800 border border-transparent hover:border-zinc-800 text-zinc-400 hover:text-zinc-100 transition-all cursor-pointer disabled:opacity-50 shrink-0"
      title="Refresh repository"
    >
      <RefreshCw class="w-3.5 h-3.5 {isLoading ? 'animate-spin text-cyan-400' : ''}" />
    </button>

    <div class="h-4 w-px bg-zinc-800 mx-0.5 hidden sm:block shrink-0"></div>
    ...
    <div class="relative tools-menu-container">
      <button
        ...
        class="p-1.5 rounded-md hover:bg-zinc-800 border border-transparent hover:border-zinc-800 text-zinc-400 hover:text-zinc-100 transition-all cursor-pointer {showToolsMenu ? 'bg-zinc-800 text-white border-zinc-700' : ''}"
      ...
      {#if showToolsMenu}
        <div class="absolute right-0 top-full mt-1.5 w-64 rounded-xl bg-zinc-900/95 border border-zinc-800 shadow-2xl backdrop-blur-xl p-1.5 z-50 text-xs font-sans animate-in fade-in slide-in-from-top-1 duration-150">
          ...
          {#if onOpenGuide}
            <button
              ...
              class="w-full flex items-center justify-between px-2.5 py-1.5 rounded-md hover:bg-cyan-950/40 text-zinc-300 hover:text-cyan-200 transition-colors cursor-pointer"
            >
              <div class="flex items-center gap-2">
                <BookOpen class="w-3.5 h-3.5 text-cyan-400" />
                <span>Playbook & Recipes</span>
              </div>
              <span class="text-[10px] font-mono text-zinc-500">F1</span>
            </button>
          {/if}
          {#if onOpenPlaybook}
            <button
              onclick={() => { showToolsMenu = false; onOpenPlaybook(); }}
              class="w-full flex items-center justify-between px-2.5 py-1.5 rounded-md hover:bg-amber-950/40 text-zinc-300 hover:text-amber-200 transition-colors cursor-pointer"
            >
              <div class="flex items-center gap-2">
                <LifeBuoy class="w-3.5 h-3.5 text-amber-400" />
          ...
          {#if onPush}
            <button
              ...
              class="w-full flex items-center justify-between px-2.5 py-1.5 rounded-md hover:bg-zinc-800 text-zinc-300 hover:text-white transition-colors cursor-pointer"
            >
              <div class="flex items-center gap-2">
              ...
              {#if currentBranch && currentBranch.ahead_count > 0}
                <span class="text-[10px] font-mono text-emerald-400 font-bold">+{currentBranch.ahead_count}</span>
              {/if}
            </button>
          {/if}
          ...
          {#if onOpenPalette}
            ...
            >
              <div class="flex items-center gap-2">
                <Command class="w-3.5 h-3.5 text-zinc-400" />
                <span>Command Palette</span>
              </div>
              <span class="text-[10px] font-mono text-zinc-500">Ctrl+K</span>
            </button>
          {/if}
          ...
          {#if onOpenTimeMachine}
            ...
            >
              ...
              <span class="text-[10px] font-mono text-zinc-500">Ctrl+Z</span>
            </button>
          {/if}
          ...
          {#if onOpenBisect}
            <button
              ...
              class="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md hover:bg-zinc-800 text-zinc-300 hover:text-white transition-colors cursor-pointer"
            >
              <Bug class="w-3.5 h-3.5 text-purple-400" />
          ...
          <button
            ...
            class="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md hover:bg-zinc-800 text-zinc-300 hover:text-emerald-300 transition-colors cursor-pointer"
          >
          ...
          <div class="px-2.5 py-1.5">
            <div class="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-1.5 flex items-center justify-between">
              ...
              <span class="font-mono text-zinc-400">{commitLimit === 0 ? 'All' : commitLimit}</span>
            </div>
            <div class="grid grid-cols-4 gap-1 bg-zinc-950 p-1 rounded-md border border-zinc-800">
              {#each [500, 2000, 5000, 0] as lim}
                <button
                  onclick={() => onLimitChange(lim)}
                  class="py-0.5 rounded text-[10px] font-mono transition-colors text-center cursor-pointer {commitLimit === lim ? 'bg-cyan-600 text-white font-semibold' : 'text-zinc-400 hover:text-zinc-200'}"
                >
                  {lim === 0 ? 'All' : lim}
                </button>
              {/each}
    ...
    {#if onOpenAuth}
      <button
        onclick={onOpenAuth}
        class="flex items-center gap-1.5 p-1 sm:px-1.5 sm:py-1 rounded-md border transition-all cursor-pointer group shrink-0 {activeAccount ? 'bg-indigo-950/40 border-indigo-500/30 text-indigo-200 hover:bg-indigo-900/60' : 'hover:bg-zinc-800 border-transparent text-zinc-400 hover:text-white'}"
      ...
      >
        {#if activeAccount?.avatar_url}
          <img src={activeAccount.avatar_url} alt="Avatar" class="w-4 h-4 rounded-full border border-indigo-400/50 shrink-0" />
        {:else}
          <svg class="w-3.5 h-3.5 {activeAccount ? 'text-indigo-400' : 'text-zinc-400'} shrink-0" viewBox="0 0 24 24" fill="currentColor">
