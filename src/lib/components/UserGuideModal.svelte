...
{#if isOpen}
  <div
    class="fixed inset-0 z-50 bg-black/75 backdrop-blur-md flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-150"
    role="dialog"
    aria-modal="true"
  >
    <!-- Modal Container -->
    <div
      class="w-full max-w-5xl h-[85vh] bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden text-zinc-200 font-sans"
    >
      <!-- Top Header -->
      <header class="h-14 px-6 border-b border-zinc-800/80 bg-zinc-900/60 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-sm font-bold text-zinc-100 flex items-center gap-2">
              ...
              <span class="text-[10px] px-2 py-0.5 rounded-full bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 font-mono">
                Sổ tay Thực chiến
              </span>
            </h2>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="relative w-64">
            ...
            <input
              ...
              class="w-full pl-8 pr-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded-lg text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-cyan-500/60 transition-colors"
            />
          </div>
      ...
      <!-- Category Tabs -->
      <div class="px-6 border-b border-zinc-800/80 bg-zinc-900/30 flex items-center gap-2 shrink-0">
        <button
          onclick={() => selectTab('recipes')}
          class="px-4 py-2.5 text-xs font-semibold flex items-center gap-2 border-b-2 transition-all cursor-pointer {activeTab === 'recipes' ? 'border-cyan-400 text-cyan-300' : 'border-transparent text-zinc-400 hover:text-zinc-200'}"
        >
        ...
        <button
          ...
          class="px-4 py-2.5 text-xs font-semibold flex items-center gap-2 border-b-2 transition-all cursor-pointer {activeTab === 'features' ? 'border-cyan-400 text-cyan-300' : 'border-transparent text-zinc-400 hover:text-zinc-200'}"
        >
        ...
        <button
          ...
          class="px-4 py-2.5 text-xs font-semibold flex items-center gap-2 border-b-2 transition-all cursor-pointer {activeTab === 'shortcuts' ? 'border-cyan-400 text-cyan-300' : 'border-transparent text-zinc-400 hover:text-zinc-200'}"
        >
      ...
      <div class="flex-1 flex overflow-hidden">
        {#if activeTab === 'shortcuts'}
          ...
          <div class="flex-1 p-6 overflow-y-auto space-y-6">
            <div>
              <h3 class="text-base font-bold text-zinc-100 mb-1">Bảng phím tắt toàn hệ thống</h3>
              <p class="text-xs text-zinc-400">Tăng tốc độ thao tác Git tối đa mà không cần rời tay khỏi bàn phím.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              {#each shortcuts as sc}
                <div class="p-3.5 rounded-xl bg-zinc-900/60 border border-zinc-800/80 flex items-center justify-between">
                  <span class="text-xs text-zinc-300 font-medium">{sc.desc}</span>
                  <kbd class="px-2.5 py-1 rounded bg-zinc-800 border border-zinc-700 text-cyan-300 font-mono text-xs font-bold shadow-xs">
                    {sc.key}
                  </kbd>
                </div>
              {/each}
        ...
        {:else}
          <!-- Left List (Items) -->
          <aside class="w-80 border-r border-zinc-800/80 bg-zinc-950/60 flex flex-col shrink-0 overflow-y-auto p-3 space-y-1.5">
            {#each filteredItems as item}
              {@const Icon = item.icon}
              <button
                onclick={() => (selectedItemId = item.id)}
                class="w-full text-left p-3 rounded-xl border transition-all cursor-pointer flex items-start gap-3 {selectedItemId === item.id ? 'bg-cyan-950/30 border-cyan-600/50 shadow-md shadow-cyan-950/20' : 'bg-zinc-900/40 border-zinc-800/60 hover:bg-zinc-900 hover:border-zinc-700'}"
              >
                <div class="p-2 rounded-lg {selectedItemId === item.id ? 'bg-cyan-500/20 text-cyan-400' : 'bg-zinc-800 text-zinc-400'} shrink-0 mt-0.5">
                  <Icon class="w-4 h-4" />
                </div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between gap-1 mb-0.5">
                    <span class="text-xs font-bold truncate {selectedItemId === item.id ? 'text-cyan-200' : 'text-zinc-200'}">
                      {item.title}
                    </span>
                  </div>
                  ...
                  {#if item.badge}
                    <span class="inline-block mt-1.5 text-[9px] font-mono px-1.5 py-0.5 rounded bg-amber-500/10 border border-amber-500/30 text-amber-300">
                      {item.badge}
                    </span>
                  {/if}
                </div>
          ...
          <!-- Right Detail Panel -->
          <main class="flex-1 p-6 md:p-8 overflow-y-auto space-y-6 bg-zinc-950">
            {#if selectedItem}
              {@const MainIcon = selectedItem.icon}
              <!-- Header of Selected Item -->
              <div class="space-y-2 border-b border-zinc-800 pb-5">
                ...
                <h1 class="text-xl font-bold text-zinc-100 flex items-center gap-2.5">
                  <MainIcon class="w-5 h-5 text-cyan-400 shrink-0" />
              ...
              <div class="space-y-3 pt-2">
                <h3 class="text-sm font-bold text-zinc-200 flex items-center gap-2">
                  <Play class="w-4 h-4 text-cyan-400 fill-current" />
                  ...
                  {#each selectedItem.steps as step, idx}
                    <div class="p-4 rounded-xl bg-zinc-900/60 border border-zinc-800/80 space-y-2">
                      <div class="flex items-center gap-2.5">
                        <span class="w-5 h-5 rounded-full bg-cyan-500/20 border border-cyan-500/40 text-cyan-300 text-[11px] font-bold flex items-center justify-center">
                          {idx + 1}
                        </span>
                        <h4 class="text-xs font-bold text-zinc-100">{step.title}</h4>
                      </div>
                      <p class="text-xs text-zinc-300 whitespace-pre-line leading-relaxed pl-7.5">
                        {step.desc}
                      </p>
                      {#if step.tip}
                        <div class="ml-7.5 p-2.5 rounded-lg bg-amber-950/30 border border-amber-800/40 text-[11px] text-amber-300 flex items-start gap-2">
                          <Lightbulb class="w-3.5 h-3.5 shrink-0 mt-0.5 text-amber-400" />
                          <span>{step.tip}</span>
                        </div>
                      {/if}
                    </div>
              ...
              {#if selectedItem.proTips && selectedItem.proTips.length > 0}
                <div class="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2">
                  ...
                  <ul class="space-y-1.5 pl-6 list-disc text-xs text-zinc-300">
