...
<div class="p-6 space-y-4">
  {#if !deviceFlowInfo}
    <div class="text-center py-5 space-y-4">
      <div class="w-14 h-14 mx-auto rounded-2xl bg-neutral-950 border border-neutral-800 flex items-center justify-center text-neutral-100 shadow-lg">
      ...
      <div class="space-y-1">
        <h3 class="text-sm font-semibold text-neutral-100">Đăng nhập tài khoản GitHub (OAuth)</h3>
      ...
      <div class="p-3 bg-amber-950/30 border border-amber-500/30 rounded-xl text-xs text-amber-200 text-left space-y-1">
        ...
        <p class="text-[11px] text-neutral-300 leading-relaxed">
          Đăng nhập qua Web chỉ cấp quyền đọc hồ sơ. Nếu bạn đồng bộ với <strong>Repository Private</strong> và gặp lỗi <code class="text-amber-300 bg-amber-950/80 px-1 py-0.5 rounded">403 Write access not granted</code>, vui lòng chuyển sang tab <strong>Personal Token / PAT</strong> để cấp đủ quyền.
        </p>
      </div>

      {#if devicePollError}
        <div class="p-3 bg-red-950/40 border border-red-800/60 rounded-xl text-xs text-red-300 text-left">
          {devicePollError}
        </div>
      {/if}
      ...
      <button
        ...
        class="w-full py-2.5 px-4 bg-neutral-100 hover:bg-white text-neutral-900 rounded-xl font-medium text-xs shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
      >
        {#if isStartingDeviceFlow}
          <div class="w-4 h-4 border-2 border-neutral-800 border-t-transparent rounded-full animate-spin"></div>
  ...
  {:else}
    ...
    <div class="space-y-4 py-2 text-center">
      <div class="space-y-1">
        <span class="text-xs text-neutral-400">Mã xác thực của bạn:</span>
        <div class="text-2xl font-mono font-bold tracking-widest text-indigo-400 bg-neutral-950 py-3 px-4 rounded-xl border border-indigo-500/30 select-all shadow-inner">
      ...
      {#if authSuccessProfile}
        <div class="p-3.5 bg-emerald-950/50 border border-emerald-500/50 rounded-xl flex items-center justify-center gap-3 animate-in zoom-in-95 duration-200">
          ...
          {/if}
          <div class="text-left">
            <span class="text-xs font-bold text-emerald-300 block">✓ Xác thực thành công!</span>
            <span class="text-[11px] text-emerald-400/90">Xin chào @{authSuccessProfile.username}</span>
          </div>
        </div>
      {:else}
        <div class="space-y-2 pt-1">
          <button
            ...
            class="w-full py-2 px-3 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 text-xs font-medium rounded-lg border border-neutral-700 transition-colors flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
          >
            {#if isManualChecking}
              <div class="w-3.5 h-3.5 border-2 border-neutral-300 border-t-transparent rounded-full animate-spin"></div>
