...
{#if onSmartSync}
  ...
  >
    ...
    {#if isBehind}
      <span class="flex items-center text-[10px] font-bold text-amber-600 dark:text-amber-400 font-mono">
...
{#if onPush}
  {#if currentBranch && !currentBranch.upstream_name}
    ...
    >
      ...
      <span class="text-[11px] font-semibold">{isPushing ? localeState.t('toolbar.publishing') : localeState.t('toolbar.publish')}</span>
    </button>
  {:else}
    <button
      onclick={onPush}
      disabled={isPushing || (currentBranch && currentBranch.ahead_count === 0)}
      class="flex items-center gap-1 p-1.5 rounded-md border text-xs transition-all {currentBranch && currentBranch.ahead_count > 0
...
<button
  onclick={onRefresh}
  disabled={isLoading}
  class="p-1.5 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 border border-transparent hover:border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-all cursor-pointer disabled:opacity-50 shrink-0"
...
<!-- Active Hotfix Banner (Only when hotfix branch is actively checked out) -->
